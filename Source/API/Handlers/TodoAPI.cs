...
public static class TodoApi
{
    ...
    internal static async Task<IResult> GetAllTodoItems(IDbContextFactory<DatabaseContext> dbContextFactory, ClaimsPrincipal user, [FromQuery(Name = "page")] int? page = 1, [FromQuery(Name = "pageSize")] int? pageSize = 10)
    {
    ...
    internal static async Task<IResult> GetTodoItemById(IDbContextFactory<DatabaseContext> dbContextFactory, ClaimsPrincipal user, int id)
    {
        using var dbContext = dbContextFactory.CreateDbContext();
    ...
    internal static async Task<IResult> CreateTodoItem(IDbContextFactory<DatabaseContext> dbContextFactory, ClaimsPrincipal user, TodoItemInput todoItemInput, IValidator<TodoItemInput> todoItemInputValidator)
    {
    ...
    internal static async Task<IResult> UpdateTodoItem(IDbContextFactory<DatabaseContext> dbContextFactory, ClaimsPrincipal user, int id, TodoItemInput todoItemInput)
    {
        using var dbContext = dbContextFactory.CreateDbContext();
    ...
    internal static async Task<IResult> DeleteTodoItem(IDbContextFactory<DatabaseContext> dbContextFactory, ClaimsPrincipal user, int id)
    {
        using var dbContext = dbContextFactory.CreateDbContext();
        if (await dbContext.TodoItems.FirstOrDefaultAsync(t => t.User.Username == user.FindFirst(ClaimTypes.NameIdentifier)!.Value && t.Id == id) is TodoItem todoItem)
        {
